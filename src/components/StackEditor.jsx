...
const AreaField = ({ control, name, placeholder, className }) => (
  <FormField control={control} name={name} render={({ field }) => (
    <FormItem><FormControl><Textarea className={className || "min-h-[50px] text-sm resize-none bg-slate-50 font-serif"} placeholder={placeholder} {...field} /></FormControl><FormMessage /></FormItem>
  )} />
);
...
function FoundationCard({ index, control, onFlushStatus }) {
  ...
  return (
    <div className={`rounded-xl overflow-hidden transition-all duration-300 ${open ? 'shadow-xl ring-1 ring-slate-900/5 bg-white' : 'shadow-sm bg-white border'}`}>
      <div className="w-full p-4 flex items-center justify-between bg-emerald-50/50 border-b border-emerald-100">
        <button type="button" onClick={() => setOpen(!open)} className="flex items-center gap-3 hover:opacity-80 transition-opacity">
          <div className="h-6 w-6 rounded flex items-center justify-center text-xs font-bold bg-emerald-700 text-white">I</div>
          <span className="text-xs font-bold uppercase tracking-wider text-emerald-700">Introduction</span>
...
function CantileverCard({ index, control, bodyNumber, onFlushStatus }) {
  const [open, setOpen] = useState(true);
      ...
      {open && (
        <div className="p-4 space-y-4">
          <div>
            ...
            <AreaField control={control} name={`segments.${index}.teel.topic`} placeholder="Your topic sentence for this paragraph..." className="min-h-[50px] text-sm resize-none bg-slate-50 font-serif" />
          </div>
          <div>
            <LabelBadge color="text-slate-500" label="Evidence Sentence" />
            <AreaField control={control} name={`segments.${index}.teel.blocks.0.evidence`} placeholder="Your evidence for this paragraph..." className="min-h-[50px] text-sm resize-none bg-slate-50 font-serif" />
          </div>
          <div>
            <LabelBadge color="text-slate-500" label="Explanation Sentence" />
            <AreaField control={control} name={`segments.${index}.teel.blocks.0.explanation`} placeholder="Your explanation of the evidence..." className="min-h-[50px] text-sm resize-none bg-slate-50 font-serif" />
          </div>
          <div>
            <LabelBadge color="text-slate-500" label="Link Sentence" />
            <AreaField control={control} name={`segments.${index}.teel.link`} placeholder="Link back to the essay question or transition to the next paragraph..." className="min-h-[50px] text-sm resize-none bg-slate-50 font-serif" />
          </div>
        </div>
      )}
    </div>
        ...
        <div className="max-w-2xl mx-auto space-y-6">
          {(() => {
            ...
            return fields.map((item, i) => {
              ...
              bodyNum++;
              return <CantileverCard key={item.id} index={i} control={control} bodyNumber={bodyNum} onFlushStatus={flushStatus} />;
            });
          })()}
        </div>
      </div>
